...
export default function AdminParcel() {
  ...
  const headers = useMemo(() => ({}), []); // ห้ามใส่ Content-Type เพราะ FormData ต้องให้ browser set เอง
  ...
  // ===== Load tenants (filtered by condoId) =====
  useEffect(() => {
    if (!condoId) return;
    const run = async () => {
      ...
      setErr("");
      try {
        const r = await fetch(`${API}/admin/tenants?condoId=${encodeURIComponent(condoId)}`, { headers });
  ...
  const fetchHistory = useCallback(async () => {
    ...
    setErr("");
    try {
      const r = await fetch(`${API}/admin/parcel/history?condoId=${encodeURIComponent(condoId)}`, { headers });
      const data = await r.json().catch(() => ({}));
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="bg-slate-100 rounded-2xl p-1 flex">
              <button
                ...
                className={`px-4 py-2 rounded-2xl font-black transition ${activeTab === "send"
              ...
              <button
                ...
                className={`px-4 py-2 rounded-2xl font-black transition ${activeTab === "history"
                  ? "bg-white shadow text-slate-900"
                  : "text-slate-600 hover:text-slate-900"
                  }`}
              >
              ...
              <div className="p-6 md:p-8">
                ...
                <div className="mt-6">
                  ...
                  <div
                    ...
                    onDrop={onDrop}
                    onClick={() => fileInputRef.current?.click()}
                    className={`rounded-3xl border-2 border-dashed p-5 md:p-6 cursor-pointer transition ${preview
